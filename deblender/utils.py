...
def restore_session(sess, ckptdir):
    ...
    """
    meta_graph_all = [os.path.join(ckptdir, file) for file
        in os.listdir(ckptdir) if file.endswith('.meta')]
    meta_graph_all.sort(reverse = True)
    meta_graph = meta_graph_all[0]    
    restorer = tf.train.import_meta_graph(meta_graph)
    print(tf.train.latest_checkpoint(ckptdir))
