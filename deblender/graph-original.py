...
#        logits_fake_2 = self.d(self.y2)
        ...
        '''
        d_loss_fake = tf.losses.sigmoid_cross_entropy(tf.zeros_like(logits_fake_1), logits_fake_1)
        ...
        '''
        g_loss = tf.losses.sigmoid_cross_entropy(tf.ones_like(logits_fake_1), logits_fake_1)
        '''
...
#                )
                blended, true_x, true_y, gan_x = self.sess.run(
                    [self.blended, self.x1, self.x2, self.y1]
                )
...
#                make_plot(blended, true_x, true_y, gan_x, gan_y, savedir, batch)
                make_plot(blended, true_x, true_y, gan_x, savedir, batch)
                batch += 1
