...
from .utils import *
from .plotting import make_plot, make_hist
...
#        self.y1, self.y2 = self.g(self.blended)
        self.y1 = self.g(self.blended)

        self.logits_real_1 = self.d(self.x1, reuse=False)
        self.logits_real_2 = self.d(self.x2)
        self.logits_fake_1 = self.d(self.y1)
#        logits_fake_2 = self.d(self.y2)
        self.logits_blended = self.d(self.blended)

        d_loss_real = (tf.losses.sigmoid_cross_entropy(
            tf.ones_like(self.logits_real_1), self.logits_real_1)
            + tf.losses.sigmoid_cross_entropy(
            tf.ones_like(self.logits_real_2), self.logits_real_2))
        '''
        ...
        '''
        d_loss_fake = tf.losses.sigmoid_cross_entropy(tf.zeros_like(self.logits_fake_1), self.logits_fake_1)
        ...
        '''
        g_loss = tf.losses.sigmoid_cross_entropy(tf.ones_like(self.logits_fake_1), self.logits_fake_1)
        '''
        ...
        '''
        self.vgg_loss = tf.losses.mean_squared_error(self.vgg(self.x1), self.vgg(self.y1))
...
#        image_summary_y2 = tf.summary.image("Image_Y2", self.y2)
    ...
    def train(self, n_batches=400000, n_burn_in_batches=4000,
        summary_interval=100, ckpt_interval=80000, progress_bar=True):
    ...
    def image_evaluate(self, savedir='/data/predictions'):
        restore_session(self.sess, self.ckptdir)
...
#                )
                blended, true_x, true_y, gan_x, d_x, d_y, d_blended, d_gan_x = self.sess.run(
                    [self.blended, self.x1, self.x2, self.y1, self.logits_real_1, self.logits_real_2, self.logits_blended, self.logits_fake_1])
                x_pos += np.sum(d_x>0)
                y_pos += np.sum(d_y>0)
                blended_pos += np.sum(d_blended>0)
                gan_x_pos += np.sum(d_gan_x>0)
                n_total += blended.shape[0]
#                true_x, true_y, gan_x, gan_y = map(
#                    lambda x: (x + 1)/2, [true_x, true_y, gan_x, gan_y]
#                )
                true_x, true_y, gan_x = map(
                    lambda x: (x + 1)/2, [true_x, true_y, gan_x]
                )
#                make_plot(blended, true_x, true_y, gan_x, gan_y, savedir, batch)
                if batch < 200:
                    make_plot(blended, true_x, true_y, gan_x, savedir, batch, d_x, d_y, d_blended, d_gan_x)
                if batch < 4:
                    make_hist(blended, true_x, true_y, gan_x, savedir, batch)
        ...
        print('x_pos:', x_pos)
        print('y_pos:', y_pos)
        print('blend_pos:', blended_pos)
        print('gan_x_pos:', gan_x_pos)
